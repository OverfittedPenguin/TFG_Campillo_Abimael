...
class NLP_STG3:
    ...
    def DYNAMIC_EQUATIONS(w,ac,at,sim):
        ...
        # PROPULSIVE FORCE AND MOMENT
        T_max,M_T_max = ac.PROPULSIVE_FORCES_MOMENTS(V,ac.RPM,rho,alpha)
        T_max = ca.if_else(x7 > (ac.BEM), T_max, 0.0)
        M_T_max = ca.if_else(x7 > (ac.BEM), M_T_max, 0.0)
        T = u1 * T_max
        M_T = u1 * M_T_max
        Fb_prop = ca.vertcat(T, 0.0)
        ...
        qk = Mb_total / Iyy
        thetak = x3
        mk = ca.if_else(x7 > ac.BEM, -ac.SFC, 0.0) # SFC.
    ...
    def COST_FUNCTIONAL(w,ac,at,sim):
        ...
        for k in range(N-1):
            ...
            # Running cost at instants i and j.
            Li = wg_dot*gi_dot + wg_sp*spi + wg_dtps*dtps_dot + wg_de*de_dot + wg_de_sat*desi
            Lj = wg_dot*gj_dot + wg_sp*spj + wg_dtps*dtps_dot + wg_de*de_dot + wg_de_sat*desj
...
class NLP_STG31:
    ...
    def CONSTRAINTS_AND_BOUNDS(x,u,utf,ac,at,sim):
        ...
        for k in range(N-1):
            wi = w[9*k:9*(k+1)]
            wj = w[9*(k+1):9*(k+2)]
            fi = NLP_STG31.DYNAMIC_EQUATIONS(wi,ac,at,sim)
            fj = NLP_STG31.DYNAMIC_EQUATIONS(wj,ac,at,sim)               
            g, lbg, ubg = NLP_STG31.DYNAMIC_CONSTRAINTS(wi[:7],wj[:7],fi[:7],fj[:7],dT,tF)
            g_dyn.append(g)
            lbg_dyn.append(lbg)
            ubg_dyn.append(ubg)
        g_dyn = sum(g_dyn,[])
        ...
        # PATH CONSTRAINTS HANDLING
        g_path, lbg_path, ubg_path = NLP_STG31.PATH_CONSTRAINTS(w,sim,ac,N)

        # INITIAL STATE CONSTRAINTS HANDLING
        g_0, lbg_0, ubg_0 = NLP_STG31.INITIAL_STATE_CONSTRAINTS(w,sim.w0)

        # SIMPLE BOUNDS FOR STATES AND CONTROL
        lbx, ubx = NLP_STG31.SIMPLE_BOUNDS(lb,ub,sim,N)
