...
After the state space is set, the constraints can be defined for the problem. At this point, previous remarks on constraint definition towards \gls{ocp} notation could be recovered. That means that constraints are described following the prestablished order, which starts by dynamic constraints. Since dynamic constraints have been defined in section \ref{sec:EOM}, they have to be transcribed using state space notation, which led to the following set of expressions.\footnote{From now on, sine operation $\sin()$ is denoted as $s$ and, cosine operation $\cos()$ is denoted as $c$, as clarified in nomenclature section This has been done for an easy readability of some equations and operations.}
...
\coloneqq
\begin{bmatrix}
    f_1(x_1,x_2,x_3,x_4,x_6,x_7,u_1,u_2) \\
    f_2(x_1,x_2,x_3,x_4,x_6,x_7,u_2) \\
    f_3(x_1,x_2,x_6,u_1,u_2) \\
    f_4(x_3) \\
    f_5(x_1,x_2,x_4) \\
    f_6(x_1,x_2,x_4) \\
    -SFC
...
Once dynamic constraints have been fully described, its time to set the path constraints and define them using state space notation. As said before, path constraints are the ones that 
