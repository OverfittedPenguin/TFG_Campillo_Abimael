...
\begin{equation}
    \begin{aligned}
        \mathcal{L}^k & \coloneqq L(\mathbf{x}^k, \mathbf{u}^k) \\
        J & \coloneqq \frac{\Delta t}{2} \sum_{k=0}^{N-1} \left( \mathcal{L}^k + \mathcal{L}^{k+1} \right)
    \end{aligned}
    \label{eq:Benchmark_NLP_Cost_Final}
\end{equation}


Next, the \gls{nlp} problem can be defined properly using almost CasADi notation -since it describes the minimisation of a cost functional defined for all symbolic variables form state-control vector $w$ subject to a set of constraints and simple bounds-. 

\begin{equation}
    \begin{aligned}
        & \min_{\mathbf{w}} \quad J(\mathbf{w}) \coloneqq \frac{\Delta t}{2}\sum_{k=0}^{N-1} \left(\mathcal{L}^k + \mathcal{L}^{k+1}\right) \\
        ...
        \text{where:} \quad & \mathbf{g}(\mathbf{w}) \coloneqq 
        \begin{bmatrix} 
            \mathbf{d}^0 \\ \vdots \\ \mathbf{d}^{N-1} \\ \hline 
            \mathbf{h}^0 \\ \vdots \\ \mathbf{h}^N \\ \hline 
            \mathbf{\varphi}_0 
...
The algorithm that has been implemented to solve the \gls{nlp} problem using \gls{ipopt} solver is the following one.
