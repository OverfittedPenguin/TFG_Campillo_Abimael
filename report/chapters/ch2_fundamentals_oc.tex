...
\begin{itemize}
    ...
    \begin{itemize}
        \item Terminal cost is the contribution that only penalises the final value of state variables. Is represented by $\Phi[y(t_f),u(t_f),t_f]$ and, if cost objective $J(y,u) = \Phi[y(t_f),t_f]$, it is said that the whole cost functional is in its Mayer form.
...
\begin{equation}
    \begin{aligned}
        ...
        where & \\
        & f_{k} =f(y_{k} ,u_{k} ,t_{k} )\text{ for each } k\in [0,N-1]
    \end{aligned}
    \label{eq:DCM_ODEs}
\end{equation}
