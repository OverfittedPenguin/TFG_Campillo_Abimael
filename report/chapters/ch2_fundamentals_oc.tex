...
\begin{itemize}
    ...
    \begin{itemize}
        \item Terminal cost is the contribution that only penalises the final value of state variables. Is represented by $\Phi[y(t_f),u(t_f),t_f]$ and, if cost objective $J(y,u) = \Phi[y(t_f),u(t_f),t_f]$, it is said that the whole cost functional is in its Mayer form.
...
\item {Indirect methods, which are based on direct evaluation of the derivative and its proximity to zero value. In this case, there is need of explicit derivation of the equations and is a method subjected to initial conditions -p.e Pontryagin's maximum principle-.}
\end{itemize}
...
\begin{equation}
    \begin{aligned}
        ...
        where & \\
        & f_{k} \coloneqq f(y_{k} ,u_{k} ,t_{k} )\text{ for each } k\in [0,N-1]
    \end{aligned}
    \label{eq:DCM_ODEs}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        ...
        \text{s.t.} \ \  & y_{k+1} =y_{k} + \frac{1}{2} h_{k} \cdotp (f_{k} +f_{k+1} ) \text{ for } k \in [0,N-1]\\
...
As said before, the \gls{ipopt} is based on a primal-dual framework that iterates between variables and multipliers by solving a linearised set of the \gls{kkt} conditions via Newton's method. This method allows to find gradients for each variable or multiplier, as stated below -with $A_k \coloneqq \nabla c(x_k)$, $W_k \coloneqq \nabla_{xx} \mathcal{L}(x_k,\lambda_k,z_k)$ and k the inner-loop counter-.
...
\FOR{$k = 0$ \TO $N$}
    ...
    \STATE \textbf{6. Update Iterates:} Compute them using equations \eqref{eq:BP_Updated_Steps}.
\ENDFOR
\IF{$k = N$}
