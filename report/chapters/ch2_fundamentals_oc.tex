...
\begin{equation}
    \begin{aligned}
        ...
        \text{s.t.} \ \  & g_j(x)\leq 0\ \text{ for each } j = 1, \dotsc, m
    \end{aligned}
    \label{eq:NLP}
\end{equation}

The \gls{nlp} problem is defined as function $f(x)$ that has to be optimised subject to a set of constraints $g_j(x)$. At this point, it has to be mentioned that any \gls{nlp} problem goes through three different phases, which are applicable to the \gls{ocp} or any optimisation problem. 

The first phase is modelling, which is, in fact, a mathematical description of a dynamic system through simple equations that allow future state predictions of a system thanks to a given set of control variables. In essence, $x$ is composed by $y$ and $u$; while state variables $y_i(t)$ are the variables that describe system conditions at a given time, control variables $u_i(t)$ are the variables that can be manipulated through time because they have an influence on the system. In other words, state variables are consequence of the control variables -e.g for a flight trajectory, the position of the aircraft could be the state variable for the \gls{tps} control variable-. 
...
Third and last phase is optimisation, which means the maximisation or minimisation of the cost function or functional associated with the system while complying with all the constraints and bounds set for a determined optimisation criteria. For example, in the brachistochrone problem introduced before only the cycloidal path complies with the minimum time optimisation criteria \cite{Bernoulli1696}.

It has to be highlighted that the \gls{ocp} is a n-dimensional interpretation of the \gls{nlp} because state and control variables are time-continuous functions and, therefore, the problem's objective is to find the $n_k$-dimensional vector of state functions $y_k(t)$ and control functions $u_k(t)$ that optimise a determined functional $F(y,u)$.
...
Once the Hamiltonian is defined, both transversality conditions can be found as the partial derivative of the Hamiltonian respect to the state variables and, the evaluation of the co-state at the end of time domain, respectively. Formulation has been adapted from \cite{Stryk1992} and \cite{Chachuat2009} and using consistent notation from the \gls{ocp}.
...
\begin{equation}
    \mathcal{L}(x,\lambda,z) \coloneqq f(x) + h(x)^T\lambda - x^Tz
...
From the previous expression, gradients could be computed. Thus, the update on variables and multipliers will be set following the set of equations \eqref{eq:BP_Updated_Steps}, once the step size parameter $\alpha_k$ is determined by a filter search or filter method. A deep explanation of the filther methods used can be found on \cite{Waechter2004} and \cite{Fletcher2002}.
