...
Eventually, the terminal cost can be added taken into account its contribution on final time-instant only. Thus, the optimal control problem can be written in its discretised form in a way such that could be solved applying a \gls{dcm}.
...
\begin{equation}
    \mathcal{L}(x,z ,\mu ):=f(x)+\sum _{i=0}^{m} z _{i} g_{i} (x)++\sum _{j=0}^{p} \mu _{j} h_{j} (x)
...
\begin{equation}
    \begin{aligned}
        \min_{x\in \mathbb{R}^{n}} \ \  & \varphi_\mu(x) := f(x) - \mu \sum_{i=0}^n ln(x^{(i)})\\
        \text{s.t.} \ \  & c(x) = 0\\
    \end{aligned}
...
\begin{equation}
    \mathcal{L}(x,\lambda,z) := f(x) + c(x)^T\lambda - x^Tz
...
\begin{subequations}
    \begin{align}
        x_{k+1} &:= x_k + \alpha_k d_x^k \\
        \lambda_{k+1} &:= \lambda_k + \alpha_k d_{\lambda}^k \\
        z_{k+1} &:= z_k + \alpha_z^k d_z^k
    \end{align}
...
\FOR{$k = 0$ \TO $N$} \STATE \textit{// Start of the Iterative Process}
...
\ENDFOR \STATE \textit{// End of the Iterative Process}
\IF{$k = N$}
...
\subsection{Implementation of IPOPT using CasADi}
...
\begin{equation}
    \begin{aligned}
        \min_{x, y, z} \quad & f(x,z) :=x^2 + 100z^2 \\
        \text{s.t.} \quad & z + (1 - x)^2 - y = 0
    \end{aligned}
    \label{eq:CasADi_Example}
\end{equation}

The directrices for the creation of \gls{nlp} problem dictionary are:
...
\begin{itemize}
    ...
    \begin{equation}
        F(x,z) := x_0^2 + 100z_0^2 + x_1^2 + 100z_1^2 + \dotsc + x_N^2 + 100z_N^2
