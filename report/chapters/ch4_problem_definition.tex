...
\begin{cases}
    ...
    -z^N - h_{ref} = 0\\
    (x^N -x^0) - (R_{ENTRY} - f_{TP}\cdot d_d) = 0
    \end{cases}
...
\begin{equation}
    ...
    \begin{bmatrix}
        -x_6^N - h_{ref}\\
        (x_5^N -x_5^0) - (R_{ENTRY} - f_{TP}\cdot d_d)
    \end{bmatrix}
    = 0
...
\begin{equation}
    L(x,u) := \sum_{k=0}^{N-1} \left(w_\gamma\cdot J_\gamma^k + w_{\dot{\gamma}}\cdot J_{\dot{\gamma}}^k + w_{\dot{\delta}_{TPS}}\cdot J_{\dot{\delta}_{TPS}} + w_{\dot{\delta}_e}\cdot J_{\dot{\delta}_e}\right)
...
\begin{equation}
    \begin{aligned}
        ...
        J(\mathbf{w}) & \coloneqq \frac{\Delta t}{2} \sum_{k=0}^{N-1} \left( L^k + L^{k+1} \right) + w_{t_F} \cdot t_F
    \end{aligned}
    \label{eq:STG1_Cost}
\end{equation}
...
\begin{equation}
    \dot{x}_7 \coloneqq 
    \begin{cases}
        -SFC & x_5 < (V_{TP} f_{TP} t_{2,min} - t_d/2) \\
        -SFC & x_5 > (V_{TP} (f_{TP} t_{2,min} + t_d/2)) \\
