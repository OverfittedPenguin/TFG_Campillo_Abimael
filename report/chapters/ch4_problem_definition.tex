...
Finally, the algorithm developed can be summarised in the following algorithm lines, see next page. As can be seen, the main structure consists in different files and functions. Essentially, it is possible to distinguish between:

\begin{itemize}
    \item \textit{simulation.py}, which is the function that extracts the information storaged in JSON file \textit{Simulation.json}. This file contains the information and problem parameters defined in \ref{fig:CONOPS}, as well as the mission bounds to the defined problem. In addition, numerical values as the number of nodes can be modified, every simulation. In the JSON file, the aircraft model can be selected and this will load automatically all geometrical, physical and operational information about the aircraft. Eventually, wind conditions could be added -they have been not used in any of the results for this thesis, which means that all results have been generated in a calm atmosphere-. 
    ...
    \item \textit{plotterfunction.py} is the postprocessing function to generate all trajectories and cost plots. It is made up by different functions to plot the whole manoeuvre trajectories and cost objective or, individual trajectories and costs per stage. It is a clean solution used to not overload main function with postprocessing activities.
    \item \textit{nonlinearprogramming.py} is the function where all \gls{nlp} dictionaries are created. Essentially, there are different functions to create state and control vectors, constraints vectors, bounds vectors and cost objective function for all symbolic variables. For each stage, a class has been created with the different functions and they only had to be called from main algorithm to create the vectors and dictionaries succesfully.
\end{itemize}
...
\STATE \quad Define number of nodes $N$ and weights for each stage penalty.
\STATE \quad Aircaft and atmosphere parameters retrieving by \textit{aircarft.py} and \textit{atmosphere.py} calls.
...
\STATE \quad Extract state and control trajectories, cost objective per iteration and final computation time.

\STATE \textbf{5. NLP Creation for STG2:} 
\STATE \quad Extract end-state from STG1 $\mathbf{x_f} \coloneqq \mathbf{w_1^N} \in \mathbb{R}^7 $.
\STATE \quad Using a static NLP solver obtain initial controls $\mathbf{u_0}\coloneqq [\delta_{TPS,0}, \delta_{e,0}]$
...
\STATE \quad Create $\mathbf{w}$, $\mathbf{g}$, $J(\mathbf{w})$ and bounds using NLP\_STG2 class and \textbf{Step 3}.

\STATE \textbf{6. Solve STG2:} 
\STATE \quad Repeat \textbf{Step 4} but with STG2 NLP dictionary.

\STATE \textbf{7. NLP Creation for STG3:} 
\STATE \quad Extract end-state from STG2 $\mathbf{x_f} \coloneqq \mathbf{w_2^N} \in \mathbb{R}^7 $.
\STATE \quad Using a static NLP solver obtain initial controls $\mathbf{u_0}\coloneqq [\delta_{TPS,0}, \delta_{e,0}]$
\STATE \quad Generate linking condition $\mathbf{w_0} \coloneqq [\mathbf{x_f}^0,\mathbf{u}_0^0, \dots, \mathbf{x_f}^N,\mathbf{u}_0^N,t_2^0] $
\STATE \quad Create $\mathbf{w}$, $\mathbf{g}$, $J(\mathbf{w})$ and bounds using NLP\_STG3 class. Repeat \textbf{Step 3}.

\STATE \textbf{6. Solve STG2:} 
\STATE \quad Repeat \textbf{Step 4} but with STG3 NLP dictionary.

\STATE \textbf{7. Post-processing:}
\STATE \quad Reconstruction of whole manoeuvre state and control trajectories. Stack all states and controls except from horizontal position which has to be stacked as follows: 
\STATE \quad \quad $\mathbf{x_5} = [\mathbf{x_{5,STG1}},\mathbf{x_{5,STG2}} + x_{5,STG1}^N, \mathbf{x_{5,STG3}} + x_{5,STG2}^N ]$
