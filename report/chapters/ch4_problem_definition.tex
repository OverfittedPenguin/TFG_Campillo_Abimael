...
\begin{table}[H]
    ...
    \begin{tabular}{llcc}
        \toprule
        \textbf{Parameter} & \textbf{Description} & \textbf{Stage} & \textbf{Units} \\ \midrule
        $t_{2,min}$ & \makecell[l]{Minimum time needed to perform the water-discharge \\ safely.} & 2 & [s] \\
        $t_{2,max}$ & \makecell[l]{Maximum time needed to perform the water-discharge \\ safely.} & 2 & [s] \\
        $t_d$ & \makecell[l]{Discharge time. Time during which discharge gates \\ are open.} & 2 & [s] \\
        $V_{TP}$ & Desired velocity to discharge safe and accurately. & 2 & [m/s] \\
        $d_d$ & \makecell[l]{Discharge distance. Bounded by $d_{2,min}$ and $d_{2,max}$. \\ $d_{2,min} \leq d_d \leq d_{2,max}$} & 2 & [m] \\
        $d_{2,min}$ & \makecell[l]{Minimum discharge distance needed. \\ $d_{2,min} \coloneqq V_{TP}\cdot t_{2,min}$} & 2 & [m] \\
        $d_{2,max}$ & \makecell[l]{Maximum discharge distance. \\ $d_{2,max} \coloneqq V_{TP}\cdot t_{2,max}$} & 2 & [m] \\
