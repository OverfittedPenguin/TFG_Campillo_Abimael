...
Next simulation tries to see which are the effects when entry and exit distances -i.e $R_{ENTRY}$ and $R_{EXIT}$- are constrained to the limit. From base case it has been seen that minimum needed distances for a $f_{TP} = 0.35$ are $R_{ENTRY} = 6500 m$ and $R_{EXIT} = 9800  m$, respectively -see table \ref{tab:Base_Case_Parameters} for reference-. This simulation uses this values to restrict the movement and adds penalty weight to end-time penalisations -$w_{t_{F,1}} = w_{t_{F,2}} = w_{t_{F,3}} = 0.5$-. 
